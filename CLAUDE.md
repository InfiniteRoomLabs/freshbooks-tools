...
- **Clean up agents once they are no longer needed** (`TaskStop`): review-lane agents right after their findings are triaged; implementers after their batch merges; QA after its verdict is acted on. Do not leave idle agents parked "just in case" -- respawning is cheap.
