...
Prebuilt binaries land on the [Releases page](https://github.com/InfiniteRoomLabs/freshbooks-tools/releases) once the `mcp/v0.1.0` and `cli/v0.1.0` tags ship (see `docs/progress.md` for status). Every release publishes, per platform (`{linux,darwin,windows} x {amd64,arm64}`), a `.tar.gz`/`.zip` archive, a shared `checksums.txt`, and an SPDX 2.3 JSON SBOM (`<archive>.sbom.json`) generated by [syft](https://github.com/anchore/syft). Verify a download against the release's `checksums.txt`:

```sh
sha256sum -c checksums.txt --ignore-missing
```

Until those tags ship, build from source:
...
Once the tags ship, `go install` also works directly from the module proxy:
