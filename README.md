...
Prebuilt binaries land on the [Releases page](https://github.com/InfiniteRoomLabs/freshbooks-tools/releases) once the `mcp/v0.1.0` and `cli/v0.1.0` tags ship (see `docs/progress.md` for status). Every release publishes, per platform (`{linux,darwin,windows} x {amd64,arm64}`), a `.tar.gz`/`.zip` archive, a shared `checksums.txt`, and a CycloneDX SBOM (`<archive>.sbom.json`) generated by [syft](https://github.com/anchore/syft). Verify a download against the release's `checksums.txt`:
