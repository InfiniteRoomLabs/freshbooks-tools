...
|---|---|---|---|
| Library | `freshbooks/` | `github.com/InfiniteRoomLabs/freshbooks-tools/freshbooks` | `freshbooks/v0.3.0` |
| MCP server | `mcp/` | `freshbooks-mcp` | `mcp/v0.1.2` |
| CLI | `cli/` | `freshbooks` | `cli/v0.1.2` |
...
Prebuilt binaries are on the [Releases page](https://github.com/InfiniteRoomLabs/freshbooks-tools/releases) (`mcp/v0.1.0` and `cli/v0.1.0` are the first). Every release publishes, per platform (`{linux,darwin,windows} x {amd64,arm64}`), a `.tar.gz`/`.zip` archive, a shared `checksums.txt`, and an SPDX 2.3 JSON SBOM (`<archive>.sbom.json`) generated by [syft](https://github.com/anchore/syft). Verify a download against the release's `checksums.txt`:
...
`go install` also works directly from the module proxy:
