...
`site/docs/` is generated by `scripts/site-sync.sh` (`mise run site-sync`) -- it copies README.md and the `docs/*.md` guides in, prepending Docusaurus front matter (`title`, `slug`, `mdx: { format: md }` so the guides, which contain `{`/`<` sequences and where `docs/cli.md` is ~8k generated lines, parse as CommonMark instead of MDX, and `custom_edit_url` pointing at the real source file) and rewriting `(docs/<name>.md)` and `(docs/<name>.md#anchor)` markdown links to the site's own slugs. Only the published guides are rewritten; a link to an unpublished `docs/*.md` is left alone so the broken-link check names the real source path. Sidebar order lives solely in `site/sidebars.js` -- the sync script writes no `sidebar_position`, which an explicit sidebar ignores anyway. `site/docs/` is gitignored and never hand-edited; nothing in `docs/*.md` or README.md itself changes.
...
mise run site          # sync, install, then the Docusaurus dev server (pnpm --dir site start)
```

`mise run site-build` joins `run_repo_wide` in `scripts/check.sh`, so `mise run check` and CI's `repo-wide` job build the docs site on every gate run. Locally that is cheap: a warm build (webpack cache present, `site/node_modules` already installed) measures ~5s, and a cold one after `pnpm exec docusaurus clear` ~26s, both well inside the 60s budget. CI is never warm -- every run is a cold build plus a full `pnpm install --frozen-lockfile`, so expect tens of seconds there even with the pnpm store cache that `ci.yml` and `pages.yml` keep (keyed on `site/pnpm-lock.yaml`); the cache saves the download, not the build. `onBrokenLinks: 'throw'` and `markdown.hooks.onBrokenMarkdownLinks: 'throw'` (`site/docusaurus.config.js`) mean a broken cross-guide link fails the gate before it ever reaches the deploy.

`.github/workflows/pages.yml` deploys on push to `main` touching anything the build reads -- `docs/**`, `README.md`, `site/**`, `scripts/site-sync.sh`, `scripts/site-build.sh`, `scripts/check.sh`, `mise.toml`, or the workflow itself -- plus `workflow_dispatch`. A `concurrency: pages` group with `cancel-in-progress: false` queues overlapping runs rather than letting two deployments race. A `build` job runs `mise run site-build` and uploads `site/build` as the Pages artifact; a `deploy` job (`environment: github-pages`, `pages: write` + `id-token: write`) publishes it via `actions/deploy-pages`. `ci.yml` is untouched -- this is a separate workflow. Like branch protection, GitHub Pages itself has to be enabled once, out of band, with `build_type=workflow` (Settings -> Pages -> Source: GitHub Actions) before this workflow can publish anything; nothing in the repo does that for you.
