...
Living status doc. Read first, update at every phase boundary. Last updated: 2026-09-03 (Phase 7 live-conformance implementation).
...
| 7 Live conformance | **implemented 2026-09-03**, in review | `phase-7/live` | nine read facts confirmed or corrected against real books; write facts DEFERRED (no sandbox); reports in `docs/phases/7/` |
...
12. **Time-entry `meta` totals are dropped** (found live 2026-09-03). `GET /timetracking/business/{id}/time_entries` answers `meta` with `total_logged`, `total_unbilled`, `total_logged_per_team_member`, and `total_logged_per_client` beside the four pagination keys; `PageMeta` keeps none of them, so `TimeEntriesService.List` cannot report a business's logged or unbilled totals. Surfacing them needs a time-entry-specific list result rather than `Page[TimeEntry]` -- a design decision, deliberately not made inside a conformance pass.
...
14. **Unmodelled wire keys on two Phase 2 read models** (QA Q3, live captures 2026-09-03): `Expense` drops 14 keys (`accountid`, `accounting_systemid`, `account_name`, `background_jobid`, `bank_name`, `billable`, `bill_matches`, `converse_projectid`, `ext_accountid`, `ext_invoiceid`, `ext_systemid`, `modern_projectid`, `potential_bill_payment`, `version`; `billable` is the one a caller would miss), `LedgerAccount` drops `category_id`, `jea_id`, `jesa_id`. Model the useful ones from `freshbooks/testdata/seed/{expenses,ledger_accounts}/`.
...
## Next action: the Phase 7 review gate

`phase-7/live` is implemented and green (`mise run check` all OK; freshbooks 91.8%, mcp 92.1%, cli 91.6%) but unreviewed and unmerged. Run the four-lane gate (code review, simplification, security in parallel, then QA) per `docs/phases/_templates/`, triage into one fix commit, re-gate, merge `--no-ff`, then retarget `GOAL.md`.

Two things a reviewer should weigh rather than rubber-stamp:

- `LedgerAccountsService.Types`/`SubTypes`/`SubType` changed return type from `json.RawMessage` to typed values. It is the change the Phase 2 comment explicitly invited once a shape was confirmed live, and both the MCP and CLI wrappers return `any` so nothing broke -- but it is a breaking library API change and belongs in the next release notes as one.
- Fact C (business-family sort direction) was NOT resolved and `Sort()` was deliberately left alone. Backlog item 1 says why.
