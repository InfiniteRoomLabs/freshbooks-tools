...
> **STATE AS OF 2026-09-03 (Phase 7, live):** the business-family filter encoding is now CONFIRMED live; the sort-direction question is NOT, and the negative result is itself the finding.
>
> - **Bare `field=value` is the business family's filter spelling -- CONFIRMED.** The authorized account has zero time entries, so a filter cannot be proved by counting results; it was proved through the validator instead. `GET /timetracking/business/{id}/time_entries?updated_since=notadate` answers **422** with `{"errno": 2001, "error": {"updated_since": "Input should be a valid datetime or date, input is too short"}}` -- the server parsed the bare parameter. The same bad value as `search[updated_since]=notadate` answers **200** with an unfiltered result: the accounting spelling is silently ignored there, not rejected. Captures: `freshbooks/testdata/seed/time_entries/`; live test `TestLiveBusinessFilterEncoding`.
> - **Sort direction is still unresolved, because the API does not validate it.** `GET /projects/business/{id}/projects` answers 200 for `sort=-updated_at`, for `sort=updated_at_desc`, *and* for `sort=no_such_field_desc`, echoing each back verbatim in `meta.sort`. There is no 4xx and no normalization to discriminate the spellings, and with zero projects on the account the orderings cannot be compared either. `Sort()` therefore keeps its `<field>_<asc|desc>` encoding; changing it on this evidence would be a guess trading one unverified spelling for another. Resolving it needs an account with at least two projects (or two time entries) whose orderings can be compared -- carried forward in `docs/progress.md`.
> - **`meta.sort` exists and was being dropped -- CORRECTED.** The business family's `meta` block carries `sort` alongside `page`/`pages`/`per_page`/`total`. `PageMeta` and `Page[T]` now have a `Sort []string` field (omitted from JSON output when empty, so the accounting family's output is unchanged). Its doc comment says plainly that the value is an echo of the request, not proof the server understood it.
> - **`meta` carries more than pagination on time entries.** `GET .../time_entries` answers `meta` with `total_logged`, `total_unbilled`, `total_logged_per_team_member`, and `total_logged_per_client` beside the four pagination keys. `PageMeta` drops all four, so `TimeEntriesService.List` cannot report a business's logged or unbilled totals. Surfacing them needs a time-entry-specific list result rather than `Page[TimeEntry]`, which is a design decision, not a conformance fix -- carried forward in `docs/progress.md`, not done here.
> - **Timestamp layouts, by live producer (fact Q).** `DateTime` accepts four wire formats; three now have named live producers, the fourth has none.
>   - `YYYY-MM-DD` -- `expenses[].date`, captured in `freshbooks/testdata/seed/expenses/list.json` and asserted by `TestLiveDateTimeFormats`. `invoices[].create_date` and `invoices[].due_date` were observed in an uncaptured CLI probe (2026-09-02): there is no `seed/invoices/` capture and no live assertion for them, because the account's invoices are real books and redacting one buys no evidence the expense row does not already give.
>   - `YYYY-MM-DD HH:MM:SS` -- `expenses[].updated` and `clients[].updated`, both captured (`seed/expenses/list.json`, `seed/accounting_clients_list.json`) and asserted by `TestLiveDateTimeFormats`. `invoices[].created_at`, and `invoices[].updated` were observed in an uncaptured CLI probe (2026-09-02) and carry no capture or assertion of their own. `clients[].signup_date` carries the same layout in the Phase 1 capture `seed/accounting_clients_list.json`.
>   - RFC 3339 -- `/auth/api/v1/users/me` `created_at`/`confirmed_at`, and every `stripe_unified` timestamp on `/payments/.../gateway`. The ledger family sends RFC 3339 *with fractional seconds* (`ledger_accounts[].updated_at`), which parses through the same layout; that field is a plain `time.Time`, not a `DateTime`.
>   - The zoneless `YYYY-MM-DDTHH:MM:SS` (`noZoneLayout`, added in Phase 2 batch c from a Projects Postman example) has **no live producer on this account**: projects and time entries are both empty, and they are the only endpoints whose examples show it. It stays INFERRED.
>   - One format the library does not name appears on the wire: a space-separated timestamp with fractional seconds, e.g. `"2026-01-02 03:04:05.678901"` (synthetic; the real stamps are not reproduced here), on `invoices[].version` and `expenses[].version`. Neither field is modelled as a timestamp (`Invoice.Version` is a `string`, `Expense` has no `Version`), so nothing decodes it today; Go's parser would accept it under `DateTimeLayout` anyway and drop the fraction on re-marshal. Recorded, not acted on.
...
>
> **STATE AS OF 2026-09-03 (Phase 7, live):** `GET /auth/api/v1/users/business/{businessId}` (the inventory's `My Team/List Staff`) answers the whole business record inside the auth family's `{"response": ...}` envelope, not just its group. Each `business_group.members[]` entry carries two keys the Postman example does not -- `identity_uuid` and `language` -- which `BusinessGroupMember` was dropping; both are now decoded. The sibling business fields the same payload carries (`account_id`, `active`, `address`, `advanced_accounting_enabled`, `billing_country_code`, `business_clients`, `business_type`, `business_uuid`, `date_format`, `first_day_of_week`, `id`, `industry`, `name`, `number_format`, `phone_number`, `status`, `timezone`) stay unreturned by `StaffService.List` on purpose; the reason is now written next to the type. Capture: `freshbooks/testdata/seed/staff/list.json`; live test `TestLiveStaffFields`.
>
> **STATE AS OF 2026-09-03 (Phase 7, live):** the `ledger` family is CONFIRMED flat, and the two taxonomy endpoints' shapes -- which had no Postman example, no docs page, and therefore no evidence at all -- are now observed.
>
> - **`/accounting/businesses/{businessUuid}/ledger_accounts/accounts` answers flat `{"data": [...]}`** with `Content-Type: application/json; charset=utf-8`. The batch-d reclassification was right. Each account carries `updated_at` as RFC 3339 with fractional seconds. Capture: `freshbooks/testdata/seed/ledger_accounts/list.json`.
> - **`/accounting/ledger_accounts/types` answers `{"data": [{"name": "asset"}, {"name": "equity"}, {"name": "expense"}, {"name": "income"}, {"name": "liability"}]}`** -- five one-key objects. The Phase 2 fixture guessed a bare string array containing "revenue"; both halves were wrong. `LedgerAccountsService.Types` now returns `[]LedgerAccountType` instead of `json.RawMessage`.
> - **`/accounting/ledger_accounts/sub_types` answers `{"data": [{"id": 1, "type": "asset", "name": "Cash & Bank", "base_number": "1000"}, ...]}`** -- ten entries, `id` a bare JSON number (the Phase 2 fixture quoted it) and a `base_number` key the fixture did not have at all. `/sub_types/{id}` answers the same object under `data`. `SubTypes` and `SubType` now return `[]LedgerAccountSubType` and `*LedgerAccountSubType`.
>
> **STATE AS OF 2026-09-03 (Phase 7, live):** `payments` and `events` are now CONFIRMED live, and `payments` carried a shape correction the Postman example could not show.
>
> - **`/payments/` is flat (business family) -- CONFIRMED.** `GET /payments/account/{accountId}/gateway` answers `{"gateway_connections": [...]}` with `Content-Type: application/vnd.api+json` and no `response`/`result` wrapper. Observed 2026-09-02, same run as J1. Capture: `freshbooks/testdata/seed/gateways/get.json`; live test `TestLiveGateways`.
> - **The gateway connection has a third, undocumented shape: `stripe_unified`.** An account onboarded through FreshBooks Payments answers `"stripe": null`, carries no `"fbpay"` key at all, and puts the whole connection under `"stripe_unified"` -- a different key set from the Postman `stripe` example (`stripe_account_id`, `publishable_key`, `account_status`, `available_onboarding_strategies`, `stripe_payouts_schedule_*`, `onboarding_completion_percent`, a `capabilities` array of `{capability, is_active}`, and four RFC 3339 timestamps). `GatewayConnection` gained `StripeUnified *StripeUnifiedConnection`; the older `Stripe` field stays, because an account onboarded before the change still answers there. Observed 2026-09-02, in the same run as the family confirmation above.
> - **`/events/` uses the accounting envelope -- CONFIRMED.** `GET /events/account/{accountId}/events/callbacks` answers `{"response":{"result":{"callbacks":[],"page":1,"pages":0,"per_page":N,"total":0}}}` with `Content-Type: application/json`, exactly as the Phase 2 example predicted. Capture: `freshbooks/testdata/seed/callbacks/list.json`; live test `TestLiveCallbacksEnvelope`.
> - **`/uploads/` is still unconfirmed:** confirming it means uploading a file, which is a write.
...
> **STATE AS OF 2026-09-03 (Phase 8, convergence):** backlog item 12 (the time-entry `meta` totals drop, flagged live in Phase 7's "`meta` carries more than pagination on time entries" bullet above) is resolved. `TimeEntriesService.ListWithTotals` runs the same one request as `List` but decodes `meta.total_logged`, `meta.total_unbilled`, `meta.total_logged_per_team_member`, and `meta.total_logged_per_client` into a new `TimeEntriesPage{Page[TimeEntry]; Totals TimeEntryTotals}` instead of dropping them; `List` is untouched. The two breakdown lists (`PerTeamMember`, `PerClient`) stay `json.RawMessage`, not a typed slice: the captured account (`freshbooks/testdata/seed/time_entries/list.json`) has zero time entries, so both are always `[]` there, and neither the Postman collection nor https://www.freshbooks.com/api/time_tracking shows a populated example -- INFERRED that the endpoint returns *something* structured in each entry, with no evidence at all of what. Resolving the element shape needs the same thing backlog item 1 (sort direction) needs: an account with real time entries.

> **STATE AS OF 2026-09-03 (Phase 8, convergence):** backlog item 14 (QA Q3's silent-drop sweep, `Expense` dropping 14 keys and `LedgerAccount` dropping 3) is resolved. All 17 keys now have struct tags, typed from the same captures QA Q3 quoted: a pointer where the capture shows `null` (INFERRED -- no evidence of the non-null shape either way), a value type with `omitempty` where the capture shows a present zero/empty value. `Expense.Billable` and `Expense.Version` are the two a caller was actually missing; the rest (`AccountingSystemID`, `AccountName`, `LegacyAccountID`, `BackgroundJobID`, `BankName`, `BillMatches`, `ConverseProjectID`, `ExtAccountID`, `ExtInvoiceID`, `ExtSystemID`, `ModernProjectID`, `PotentialBillPayment` on `Expense`; `CategoryID`, `JEAID`, `JESAID` on `LedgerAccount`) round out parity with the wire. `LedgerAccount.CategoryID` has no non-null evidence in the capture at all (every account's `category_id` is `null`); typed `*int64` to match the codebase's other `*_id` fields rather than guessing a string, but this is a coin flip the plan's own phrasing left ambiguous -- flagged for whoever next sees a populated value.
