...
>   - `YYYY-MM-DD` -- `expenses[].date`, `invoices[].create_date`, `invoices[].due_date`.
>   - `YYYY-MM-DD HH:MM:SS` -- `expenses[].updated`, `invoices[].created_at`, `invoices[].updated`, `clients[].signup_date`, `clients[].updated`.
...
>
> - **`/payments/` is flat (business family) -- CONFIRMED.** `GET /payments/account/{accountId}/gateway` answers `{"gateway_connections": [...]}` with `Content-Type: application/vnd.api+json` and no `response`/`result` wrapper. Capture: `freshbooks/testdata/seed/gateways/get.json`; live test `TestLiveGateways`.
