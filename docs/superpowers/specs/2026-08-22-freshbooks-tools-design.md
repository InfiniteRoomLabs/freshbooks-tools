...
>
> **STATE AS OF 2026-08-22 (Phase 0, inventory tool):** the "identical method and URL" claim above is wrong -- each `Single Tax` pair is a GET and a DELETE that share the same Postman request name and URL, not two copies of the same request. That is a genuine name collision, not an exact duplicate, so the inventory tool cannot silently collapse it into one entry without losing the DELETE operation. `Normalize` disambiguates: the first occurrence (GET) keeps the plain key, and the second (DELETE) gets a `" (DELETE)"` suffix. The real collection therefore normalizes to **213 distinct inventory keys**, not 211; "211" remains correct only as the count of distinct Postman *request names* before disambiguation.
