...
>   - `YYYY-MM-DD HH:MM:SS` -- `expenses[].updated` and `clients[].updated`, both captured (`seed/expenses/list.json`, `seed/accounting_clients_list.json`) and asserted by `TestLiveDateTimeFormats`. `invoices[].created_at`, `invoices[].updated`, and `clients[].signup_date` were observed in an uncaptured CLI probe (2026-09-02) and carry no capture or assertion of their own.
