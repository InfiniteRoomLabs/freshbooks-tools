...
>   - One format the library does not name appears on the wire: a space-separated timestamp with fractional seconds, e.g. `"2026-08-24 12:14:28.731850"`, on `invoices[].version` and `expenses[].version`. Neither field is modelled as a timestamp (`Invoice.Version` is a `string`, `Expense` has no `Version`), so nothing decodes it today; Go's parser would accept it under `DateTimeLayout` anyway and drop the fraction on re-marshal. Recorded, not acted on.
