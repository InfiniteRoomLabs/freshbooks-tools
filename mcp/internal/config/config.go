...
type Config struct {
	...
	// call falls back to when it omits the corresponding field.
	AccountID    string
	BusinessID   int64
	BusinessUUID string
...
// flagDefs is the flag name, its FRESHBOOKS_MCP_* env twin, and its
// default value, for every cobra flag Load resolves.
var flagDefs = []struct {
	name, env, def string
}{
	{"transport", "FRESHBOOKS_MCP_TRANSPORT", "stdio"},
	{"addr", "FRESHBOOKS_MCP_ADDR", "127.0.0.1:8080"},
	{"path", "FRESHBOOKS_MCP_PATH", "/mcp"},
	{"log-level", "FRESHBOOKS_MCP_LOG_LEVEL", "info"},
	{"log-format", "FRESHBOOKS_MCP_LOG_FORMAT", "text"},
}
...
func AddFlags(cmd *cobra.Command) {
	for _, f := range flagDefs {
		cmd.Flags().String(f.name, f.def, flagUsage[f.name])
	}
}

var flagUsage = map[string]string{
	"transport":  "transport to serve on: stdio or http",
	"addr":       "address to listen on in http mode",
	"path":       "URL path to serve the MCP endpoint on in http mode",
...
// line if they passed one, else the env twin if set, else the flag's
// built-in default.
...
func Load(cmd *cobra.Command) *Config {
	cfg := &Config{
		...
		Addr:      stringFlag(cmd, "addr", "FRESHBOOKS_MCP_ADDR"),
		Path:      stringFlag(cmd, "path", "FRESHBOOKS_MCP_PATH"),
		LogLevel:  stringFlag(cmd, "log-level", "FRESHBOOKS_MCP_LOG_LEVEL"),
		LogFormat: stringFlag(cmd, "log-format", "FRESHBOOKS_MCP_LOG_FORMAT"),
...
// understands, that LogLevel parses, and -- for stdio, which owns its own
// client for the life of the process -- that a usable token configuration
// is present. Http mode is not checked here: it ignores the token
// environment entirely, since its bearer comes from each request.
...
// transport), at LogLevel, in LogFormat.
