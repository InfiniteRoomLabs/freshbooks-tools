...
// an error, or a log. All four are registered with newSensitiveSpec, not
// newSpec: mcp.AddTool's generic dispatcher would otherwise quote a
// schema-invalid value straight into the tool result before any of this
// package's own error mapping ever runs (see newSensitiveSpec's doc
// comment and docs/phases/3/reports/security.md finding 1). The generic
// error mapping in errors.go only ever formats the *lib's* returned error
// (which never carries card data or a bearer token, per
// freshbooks/errors.go), and none of the Call closures below format `in`
// with %v/%+v -- both hold structurally, not by review.
...
var paymentOptionsSpecs = []Spec{
	newSensitiveSpec("payment_options_fb_pay_tokenize",
	...
	newSensitiveSpec("payment_options_stripe_tokenize",
	...
	newSensitiveSpec("payment_options_stripe_create_setup_intent",
	...
	newSensitiveSpec("payment_options_save_credit_card",
